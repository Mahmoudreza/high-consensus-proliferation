...
# def runIC(G, S, p=.01, pdd, pdr, pdn, prd, prr, prn, pnd, pnr, pnn):
def runIC(G, S, p=.01):
    ...
    d-- dem, r-- rep, n--neut, pdd-- propagation probability from dem to dem, ...
    ...
    while i < len(T):
        for v in G[T[i]]:  # for neighbors of a selected node
            if v not in T:  # if it wasn't selected yet
                ...
                # print(r, w)
                # if random() <= 1 - (1-p)**w: # if at least one of edges propagate influence
                if random() <= w[h_l]:  # i.e. now w is actually probability and there is only one edge between two nodes
                    # print(T[i], 'influences', v)
                    T.append(v)
                    if G.nodes[v]['color'] == 'red':
                        T_r.append(v)
                    elif G.nodes[v]['color'] == 'blue':
                            T_b.append(v)
                    else:
                        T_n.append(v)
        i += 1
    ...
    while i < len(T):
        for v in G[T[i][0]]:  # for neighbors of a selected node where T[i] = (v,t) hence T[i][0] = v
            ...
            if random() <= w:  # i.e. now w is actually probability and there is only one edge between two nodes
                ...
                index = [idx for idx, item in enumerate(T) if
                         item[0] == v]  # i.e. get the timing of infection if the node is infected
